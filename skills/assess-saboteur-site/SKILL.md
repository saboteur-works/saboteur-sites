...
The footer links `/privacy` and `/terms` from every page, so these are part of the site, not an afterthought. A published policy is a factual claim about the build — these checks test the claim.
...
| Pages are generated, not hand-written | Each legal page carries the `GENERATED by saboteur-sites/scripts/render-policies.mjs` banner. A legal page without it was hand-authored and is already drifting from every other Saboteur site. **P2.** | same |
| Pages match the templates | `node $SITES/scripts/render-policies.mjs --config ./policy.config.json --out ./src/pages --check`. Non-zero exit means someone edited the output or the shared body moved. | same |
| Feature flags match the site | `features.contact_form` true iff a Contact section exists; `features.analytics` true iff an analytics snippet is actually in the build; `features.embeds` true iff a click-to-load embed exists. **A wrong flag makes the policy false** — that is a **P1**, not a nit. | `compliance/privacy-policy/checklist.md` |
| Processor claims match the build | `npm run build && node $SITES/scripts/audit-policy-processors.mjs --config ./policy.config.json --dist ./dist`. Report undeclared and over-declared hosts separately. | `compliance/policy-tooling.md` |
| Retention claims are specific | `tokens.log_retention_days` and `form_retention_months` are numbers, not prose. Flag if the rendered page says "as long as necessary". | `compliance/privacy-policy/checklist.md` |
| US state coverage is intact | The rendered privacy page contains an appeals process, a 45-day response window, and an affirmative "we do not sell or share" statement. Missing appeals is the most commonly dropped requirement. | `compliance/us-state-privacy.md` |
| `last_updated` is not stale | If `tokens.last_updated` predates the most recent change to the site's processors or form, the policy is out of date. | same |

Do **not** file a finding for the GDPR sections being present on a US-only site. That is the deliberate posture recorded in `compliance/policy-tooling.md §Jurisdictional posture`.

For each finding in this group:
- **Category:** Legal
- **Location:** `<file:line>` or the config key
